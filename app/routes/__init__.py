...
def register_error_handlers(app):
    ...
    def bad_request(e):
        logger.info(f"400 Bad Request - Path: {request.path} - IP: {request.remote_addr}")
        return error_response(400, "Bad request. Please check your input and try again.")

    @app.errorhandler(401)
    def unauthorized(e):
        logger.warning(f"401 Unauthorized - Path: {request.path} - IP: {request.remote_addr}")
        return error_response(401, "Authentication required. Please log in.")

    @app.errorhandler(403)
    def forbidden(e):
        logger.warning(f"403 Forbidden - Path: {request.path} - IP: {request.remote_addr}")
        return error_response(403, "Access denied")

    @app.errorhandler(404)
    def not_found(e):
        logger.info(f"404 Not Found - Path: {request.path} - IP: {request.remote_addr}")
        return error_response(404, "Resource not found")

    @app.errorhandler(405)
    def method_not_allowed(e):
        logger.warning(f"405 Method Not Allowed - Path: {request.path} - Method: {request.method} - IP: {request.remote_addr}")
    ...
    def too_many_requests(e):
        ...
        logger.warning(f"429 Too Many Requests - Path: {request.path} - IP: {request.remote_addr}")
